# -*- coding: utf-8 -*-
"""
Created on Sat Jun 26 10:22:15 2021
        ...
        for i in range(self.size):
            for j in range(self.size - 1):
                ...
                and self.cells[j][i] != 0:
                    """上方數字*2，下方數值等於0"""
                    self.score = self.score + self.cells[j][i]
                    self.cells[j][i] *= 2
                    self.cells[j+1][i] = 0
...
class Game:
    ...
    def you_win(self):
        self.won = True
        print('You Win!')
        messagebox.showinfo(title = 'u win!', message = 'u win!')

    """遊戲失敗"""
    def game_over(self):
        print('Game over!')
        messagebox.showinfo(title = 'You suck!', message = 'u lose!LLLLLLLLLLLLLLLLLLLLLLLLL')
